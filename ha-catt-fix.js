...
function removeTouchControls() {
  ...
  if (controls) { controls.remove(); observer1.disconnect(); }
}
function removeStyleDiv() {
  Array.from(document.getElementsByTagName("div")).forEach(function(el) {
    if (el.innerHTML.includes("--cast-controls")) { el.remove(); observer2.disconnect(); }
  });
}
function setupMutationObservers() {
  window.observer1 = new MutationObserver(removeTouchControls);
  window.observer1.observe(document.body, { childList: true, subtree: true });
  window.observer2 = new MutationObserver(removeStyleDiv);
  window.observer2.observe(document.body, { childList: true, subtree: true });
}
function startReceiver() {
  window.castReceiverContext = cast.framework.CastReceiverContext.getInstance();
  const deviceCaps = window.castReceiverContext.getDeviceCapabilities();
  if (deviceCaps === null) return;
  ...
  try {
      ...
      console.log("Cast receiver context start error:", e);
  }
}
